 ...
 **/
public class HttpProxyServerViaSocks5ClientConnectionReadyListener implements ChannelFutureListener {

    private static final Logger logger = LoggerFactory.getLogger(HttpProxyServerViaSocks5ClientConnectionReadyListener.class);
    ...
    public void operationComplete(ChannelFuture future) {
        if(future.isSuccess()) {
            if(localChannelHandlerContext.channel().isOpen() && remoteChannelHandlerContext.channel().isOpen()) {
                ...
                NettyComponentUtil.removeAllHandler(channelPipeline, HttpResponseEncoder.class);
                NettyComponentUtil.removeAllHandler(channelPipeline, HttpProxyServerViaSocks5Handler.class);
                NettyComponentUtil.addLastHandler(channelPipeline, "proxy-forwarding-handler", new ForwardingMessageHandler(remoteChannelHandlerContext));
            } else {
    ...
    public HttpProxyServerViaSocks5ClientConnectionReadyListener(ChannelHandlerContext localChannelHandlerContext, ChannelHandlerContext remoteChannelHandlerContext, String clientName) {
