package cn.t.freetunnel.common.constants;

/**
 * @author <a href="mailto:yangjian@liby.ltd">研发部-杨建</a>
 * @version V1.0
 * @since 2022-03-10 17:36
 **/
public class NettyHandlerName {
    public static final String SOCKS5_TUNNEL_CLIENT_FORWARDING_MESSAGE_HANDLER = "socks5-tunnel-client-forwarding-message-handler";
    public static final String SOCKS5_TUNNEL_SERVER_FORWARDING_MESSAGE_HANDLER = "socks5-tunnel-server-forwarding-message-handler";
    public static final String SOCKS5_TUNNEL_SERVER_COMMAND_HANDLER = "socks5-tunnel-server-command-handler";
    public static final String SOCKS5_TUNNEL_CLIENT_COMMAND_HANDLER = "socks5-tunnel-client-command-handler";
    public static final String ENCRYPT_MESSAGE_DECODER = "encrypt-message-decoder";
    public static final String ENCRYPT_MESSAGE_ENCODER = "encrypt-message-encoder";
    public static final String LAYER_MESSAGE_DECODER = "layer-message-decoder";
    public static final String LAYER_MESSAGE_ENCODER = "layer-message-encoder";
}
